...
public class TakingTurnsQueueTests
{
    ...
    // Defect(s) Found:  Sue is returned first instead of Bob, so the queue insertion order is wrong.
    ...
    // Defect(s) Found: Sue is returned before Bob in the beginning, so the initial queue order is wrong.
    ...
    // Run 10 times.
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: Sue is returned first instead of Bob, so the turns and order logic for infinite and finite players is wrong.
    ...
    // Defect(s) Found: Sue is returned first instead of Tim, so the queue insertion order is wrong.
